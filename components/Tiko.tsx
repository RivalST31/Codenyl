...
import { TIKO_KNOWLEDGE, KnowledgeItem } from '../constants';
...
export const Tiko: React.FC<TikoProps> = ({ isOpen, onClose }) => {
  const [input, setInput] = useState('');
  const [knowledge, setKnowledge] = useState<KnowledgeItem | null>(null);
  const [response, setResponse] = useState("Hi! I'm Tiko. How can I help?");
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setIsThinking(true);
    setMood('thinking');
    setKnowledge(null); // Reset current knowledge link
    
    // Simulate processing time
    setTimeout(() => {
      const lowerInput = input.toLowerCase();
      const match = TIKO_KNOWLEDGE.find(item => 
  ...
  const getLinkStyles = (color?: string) => {
    switch(color) {
      case 'cyan': return 'text-cyan-600 border-cyan-200 hover:bg-cyan-50 focus:ring-cyan-500';
      case 'red': return 'text-red-600 border-red-200 hover:bg-red-50 focus:ring-red-500';
      case 'green': return 'text-emerald-600 border-emerald-200 hover:bg-emerald-50 focus:ring-emerald-500';
      default: return 'text-indigo-600 border-indigo-200 hover:bg-indigo-50 focus:ring-indigo-500';
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] pointer-events-none flex items-end justify-end p-4 sm:p-8">
      ...
      <div className="relative z-10 flex flex-col items-end gap-4 pointer-events-auto">
        ...
        {/* Thought Bubble */}
        <div className={`
            relative max-w-xs sm:max-w-sm bg-white p-6 rounded-3xl rounded-br-none shadow-[0_0_40px_rgba(99,102,241,0.25)] 
            transform transition-all duration-500 origin-bottom-right flex flex-col gap-3
            ${isOpen ? 'scale-100 opacity-100 translate-y-0' : 'scale-50 opacity-0 translate-y-20'}
        `}>
            <button 
                onClick={onClose}
                className="absolute -top-3 -left-3 p-2 bg-slate-100 hover:bg-slate-200 text-slate-500 rounded-full transition-colors shadow-sm z-20"
            >
                <X className="w-4 h-4" />
            </button>
            ...
            <div className="font-display font-medium text-lg leading-relaxed min-h-[3rem]">
                {isThinking ? (
                    <div className="flex gap-1.5 h-6 items-center py-2">
                        <div className="w-2 h-2 bg-indigo-500 rounded-full animate-bounce" style={{ animationDelay: '0ms' }}></div>
                        <div className="w-2 h-2 bg-indigo-500 rounded-full animate-bounce" style={{ animationDelay: '150ms' }}></div>
                        <div className="w-2 h-2 bg-indigo-500 rounded-full animate-bounce" style={{ animationDelay: '300ms' }}></div>
                    </div>
                ) : (
                    <span className="bg-gradient-to-r from-indigo-700 via-purple-700 to-indigo-700 bg-clip-text text-transparent bg-[length:200%_auto] animate-shimmer">
                        {displayedResponse}
                    </span>
                )}
            </div>

            {/* Link Button (Only show when not thinking and typing is done) */}
            {!isThinking && knowledge?.link && displayedResponse.length === response.length && (
              <a 
                href={knowledge.link.url}
                target="_blank"
                rel="noopener noreferrer"
                className={`flex items-center gap-2 px-4 py-2 rounded-xl border-2 text-sm font-bold transition-all duration-300 hover:scale-105 active:scale-95 focus:outline-none focus:ring-2 focus:ring-offset-1 ${getLinkStyles(knowledge.link.color)}`}
              >
                <span>{knowledge.link.label}</span>
                <ExternalLink className="w-3.5 h-3.5" />
              </a>
            )}
            
            {/* Bubble Tail */}
            <div className="absolute -bottom-2 right-8 w-6 h-6 bg-white rotate-45 transform skew-x-12 z-10"></div>
        </div>
        ...
        <div className="relative w-40 h-40 sm:w-48 sm:h-48">
            <div className={`w-full h-full transition-transform duration-700 ease-in-out animate-float-slow`}>
                <svg viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg" className="drop-shadow-2xl">
                    ...
                    {/* Body */}
                    <path d="M100 160C133.137 160 160 133.137 160 100C160 66.8629 133.137 40 100 40C66.8629 40 40 66.8629 40 100C40 133.137 66.8629 160 100 160Z" fill="url(#tiko_paint0_linear)" stroke="#475569" strokeWidth="2"/>
                    <defs>
                        <linearGradient id="tiko_paint0_linear" x1="100" y1="40" x2="100" y2="160" gradientUnits="userSpaceOnUse">
